...
import { testproblems, language as maplanguage } from './simplmaplang.js';

const serversArg = process.argv[2] || 'http://localhost:3000';
...
async function main() {
    for (const [name, problem] of Object.entries(testproblems)) {
        ...
        try {
            const result = await parallelSolve(problem, servers, maplanguage);
